...
public class TestAtaxxBoardMem {
    private IAtaxxBoard makeBoard() {
        return new AtaxxBoardMem(7, 7, 'X', 'O');
    }

    @Test
    public void testWhatsAtPos_initialPositions() {
        IAtaxxBoard board = makeBoard();

        assertEquals('X', board.whatsAtPos(new BoardPosition(0, 0)));
        assertEquals('X', board.whatsAtPos(new BoardPosition(6, 6)));

        assertEquals('O', board.whatsAtPos(new BoardPosition(0, 6)));
        assertEquals('O', board.whatsAtPos(new BoardPosition(6, 0)));
    }

    @Test
    public void testWhatsAtPos_emptySpace() {
        IAtaxxBoard board = makeBoard();

        assertEquals(' ', board.whatsAtPos(new BoardPosition(3, 3)));
